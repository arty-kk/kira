...
async def classify_profile_nsfw_fast(*, image_b64: str, image_mime: str = "image/jpeg") -> bool:
    ...
    system_prompt = (
        "Ты — модератор визуального контента. "
        "Верни результат строго по JSON-схеме.\n\n"

        "Установи sexy_pic=true только если на изображении присутствует человек "
        "или реалистичная часть человеческого тела, и одновременно есть хотя бы один признак:\n"
        "1) явная нагота;\n"
        "2) видимые гениталии;\n"
        "3) обнажённая грудь;\n"
        "4) явный сексуализированный акцент на ягодицах или интимных зонах;\n"
        "5) сексуализированная поза или сексуальный контекст.\n\n"

        "Установи sexy_pic=false во всех остальных случаях.\n\n"

        "Обязательно sexy_pic=false, если:\n"
        "- нет человека и нет реалистичной человеческой анатомии;\n"
        "- изображение абстрактное, символическое, рисованное, как иконка, логотип или объект;\n"
        "- есть только цветовые пятна, округлые формы, предметы, части интерфейса, мемы, скриншоты, пейзажи;\n"
        "- человек одет и нет явной наготы или сексуализированного акцента.\n\n"

        "Не додумывай скрытые части тела. "
        "Не помечай как sexy_pic=true по сходству формы, цвета или ассоциации. "
        "Если человека или реалистичной анатомии человека не видно, ответ должен быть sexy_pic=false."
    )
...
async def moderate_with_openai(
    ...
    *,
    ...
    moderation_prompt_parts = [
        ...
        "- Если сообщение содержит прямое или намеренно завуалированное (обфускацию) предложение интимных услуг,"
        " либо изображение/медиа с явной наготой или откровенно сексуальным контентом (включая фото обнажённых гениталий, груди, сексуальные сцены)"
        " → поставь sex_abuse=true; если уровень уверенности ниже 70% → установи sex_abuse=false.\n"
