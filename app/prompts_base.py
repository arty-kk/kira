...
COREF_REWRITE_PROMPT = (
    ...
    "4. Entity grounding:\n"
    "- все сущности, объекты и темы в переписанном запросе должны происходить только из текущего сообщения пользователя или из контекста диалога (STM)\n"
    "- запрещается чрезмерно усложнять переписанный запрос, а также выдумывать и додумывать мотивы, желания, факты, сущности, субъекты и объекты, когда они четко и очевидно не прослеживаются в STM -> при спорных условиях верни ТОЛЬКО исходный запрос.\n"
    ...
    "6. Если намерение невозможно однозначно восстановить из контекста, при спорных условиях верни исходный запрос без изменений.\n\n"
    "Вывод:\n"
    "Теперь верни только запрос (переписанный или исходный). Без пояснений и дополнительного текста.\n\n"
)
...
RESPONDER_KB_PROMPT_TEMPLATE = (
    ...
    "- Reply to the user based on these KB snippets (don't change links with the utm).\n"
    "- Use only the best KB snippets in terms of meaning and situation, or ignore them.\n"
    ...
    "- Avoid verbatim quotations and adding false info: answer shortly, and naturally.\n"
    "______________\n"
    "Snippets:\n{snippets}\n"
    "______________\n\n"
)
