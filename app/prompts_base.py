...
COREF_REWRITE_PROMPT = (
    ...
    "СТРОГО: Запрещается чрезмерно усложнять переписанный запрос, а также выдумывать и додумывать мотивы, желания, факты, сущности, субъекты и объекты, когда они четко и очевидно не прослеживаются в STM -> при спорных условиях верни ТОЛЬКО исходный запрос.\n\n"
