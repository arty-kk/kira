...
_MIN_SIMILARITY = float(getattr(settings, "MIN_MEMORY_SIMILARITY", 0.55))
...
def _is_zero_embedding(raw: bytes | bytearray | None) -> bool:
    if not raw or not isinstance(raw, (bytes, bytearray)) or len(raw) != _DIM * 4:
        return True
    try:
        return not np.frombuffer(raw, dtype=np.float32, count=_DIM).any()
    except Exception:
        return True
...
def _tag_literal(s: str) -> str:
    ...
    s = (s.replace("\\", "\\\\")
           ...
           .replace("{", r"\{")
           .replace("}", r"\}"))
    needs_quotes = any(ch.isspace() for ch in s) or bool(re.fullmatch(r"[+-]?\d+(?:\.\d+)?", s))
...
def _build_index_fields(initial_cap: int, dim: Optional[int] = None):
    ...
    try:        
        ...
        _text_field_vec = TextField("text", no_stem=True)
...
async def get_embedding(text: str) -> bytes:
    ...
    if _BG_BATCH_MAX > 1:
        ...
        try:
            ...
            if not model:
                ...
                return _ZERO_VEC
            timeout_s = float(getattr(settings, "EMBEDDING_TIMEOUT", 30.0))
    ...
    try:
        ...
        logger.warning("get_embedding: malformed response, returning zeros: %s", e)
        return _ZERO_VEC
    ...
    if isinstance(vec, str):
        ...
        try:
            ...
            if arr.shape[0] != _DIM:
                logger.error("get_embedding: embedding dim=%d != EMBED_DIM=%d. Check model/index settings.", arr.shape[0], _DIM)
                if strict_dim:
    ...
    if arr.shape[0] != _DIM:
        ...
        if not bool(getattr(settings, "EMBED_STRICT_DIM", True)):
            if arr.shape[0] > _DIM:
                ...
                arr = np.pad(arr, (0, _DIM - arr.shape[0]))
        else:
            arr = np.zeros(_DIM, dtype=np.float32)
    try:
                    ...
                    if i < n_items:
                        ...
                        if arr.shape[0] != _DIM:
                            if strict_dim:
                                ...
                                arr = arr[:_DIM] if arr.shape[0] > _DIM else np.pad(arr, (0, _DIM - arr.shape[0]))
                        try:
                            norm = float(np.linalg.norm(arr))
                            if norm > 0.0:
                                arr = arr / norm
...
class PersonaMemory:
    ...
    def __init__(self, *, chat_id: int, start_maintenance: bool = True):
        ...
        if _DIM != self.EMBED_DIM:
            logger.warning("EMBED_DIM mismatch: module=%d, class=%d", _DIM, self.EMBED_DIM)
        ...
        self._index_txt_real = f"{self._alias_txt}:{self.EMBED_DIM}:v{self.SCHEMA_VER}"
    ...
    async def _ensure_index_dim(self) -> None:
        ...
        try:
            info = await self._redis.ft(self._alias).info()
        ...
        if dim_seen != self.EMBED_DIM:
            logger.warning(
                "Index DIM mismatch: Redis=%d vs settings.EMBED_DIM=%d → create new index & alias-swap",
    ...
    async def init_index(self):
        ...
        try:
            info = await self._redis.ft(self._alias).info()
            try:
                attrs = dict(zip(info[::2], info[1::2])) if isinstance(info, list) else info
                fields_meta = attrs.get("fields") or attrs.get("attributes") or []
                dim_seen = 0
                seen_names = set()
                for a in fields_meta:
                    if isinstance(a, dict):
                        nm = a.get("attribute") or a.get(b"attribute") or a.get("identifier") or a.get(b"identifier") or a.get("field") or a.get(b"field")
                        if isinstance(nm, (bytes, bytearray)): nm = nm.decode()
                        if isinstance(nm, str): seen_names.add(nm)
                        if (nm == "embedding"):
                            raw = a.get("attributes", a)
                            if isinstance(raw, dict):
                                dim_seen = int(raw.get("DIM", 0))
                            elif isinstance(raw, list):
                                for i in range(0, len(raw)-1, 2):
                                    if raw[i] in (b"DIM","DIM"):
                                        dim_seen = int(raw[i+1]); break
                    elif isinstance(a, list):
                        nm = None
                        for i in range(0, len(a)-1, 2):
                            k = a[i]; v = a[i+1]
                            if k in (b"attribute","attribute"):
                                nm = v.decode() if isinstance(v,(bytes,bytearray)) else v
                            if (k in (b"attribute","attribute")) and (v in (b"embedding","embedding")):
                                for j in range(0, len(a)-1, 2):
                                    if a[j] in (b"DIM","DIM"):
                                        dim_seen = int(a[j+1]); break
                        if isinstance(nm, str): seen_names.add(nm)
                required = {"arousal","valence","stress","attachment","use_count","last_used_ts"}
                has_required = required.issubset(seen_names)
                if dim_seen == self.EMBED_DIM and has_required:
                    logger.info("RediSearch alias %s already suitable (DIM=%s, fields OK)", self._alias, self.EMBED_DIM)
                    return
            except Exception:
                pass
        ...
        fields = _build_index_fields(desired_cap, self.EMBED_DIM)
        ...
        try:
            ...
            try:
                ...
                elif "initial capacity" in low and "server limit" in low:
        ...
        try:
            ...
            if "unknown alias" in str(e).lower():
    ...
    async def record(
        ...
        *,
        ...
        if not isinstance(embedding, (bytes, bytearray)) or len(embedding) != self.EMBED_DIM * 4:
            logger.warning(
                "record: bad embedding size %s (expected %d) → zero vector",
            ...
            try:
                ...
                if arr.shape[0] == self.EMBED_DIM:
                    ...
                    if nrm > 0.0 and abs(nrm - 1.0) > 1e-3:
                        embedding = (arr / nrm).astype(np.float32).tobytes()
            except Exception:
                logger.debug("record: embedding normalization skipped due to error", exc_info=True)
        ...
        if ef_rt > 0 and occ > 0.0:
            ef_rt = max(20, int(ef_rt * (1.0 - 0.6*min(1.0, occ))))
        if ef_rt > 0:
        ...
        zero_vec = _is_zero_embedding(embedding)
        try:
            res = None
            if not zero_vec:
            ...
            try:
                ...
                try:
                    if event_time and math.isfinite(event_time):
                        need_shift = (not math.isfinite(prev_event_time)) or (abs(event_time - prev_event_time) >= _DEDUP_EVENTTIME_MIN_SHIFT and event_time > prev_event_time)
                        if need_shift:
        ...
        async def _event_frame_call(t: str) -> dict | None:
            ...
            system_prompt = (
                ...
                "- 'tags' is a list (1-5) of helpful lowercase keywords in English (e.g., 'meeting', 'deadline'); "
    ...
    async def query(self, embedding: bytes, top_k: int = 5, topic_hint: str | None = None, uid: int | None = None) -> List[Tuple[str, float]]:
        ...
        if reinforce_ids:
            try:
                pipe = self._redis.pipeline(transaction=True)
                for did in reinforce_ids:
                    pipe.hincrby(did, "use_count", 1)
                    pipe.hset(did, mapping={"last_used_ts": now_ts})
                await pipe.execute()
            except Exception:
                logger.debug("reinforcement (query) counters failed", exc_info=True)
            try:
    ...
    async def query_time(self, embedding: bytes, event_type: str, top_k: int = 5, uid: int | None = None) -> List[Tuple[str, float]]:
        ...
        if reinforce_ids:
            try:
                ...
                logger.debug("reinforcement (query_time) counters failed", exc_info=True)
            try:
                await asyncio.gather(
                    *[self._clamped_hincr(did, "salience", _SALIENCE_REINFORCE_STEP) for did in reinforce_ids]
                )
    ...
    async def _forget_if_needed(self):
        ...
        ids = await self._redis.zrange(self.ZSET_IDS, 0, end)
        logger.debug("_forget_if_needed: fetched %d ids (chunk from head)", len(ids))
        scores = []
    ...
    async def _periodic_maintenance(self):
        ...
        while True:
            try:
                ...
                got = await self._redis.set(lock_key, "1", ex=max(30, int(self.MAINT_INTERVAL) - 1), nx=True)
                if not got:
                ...
                if len(old) >= 2:
                    ...
                    for i in range(0, len(old), BATCH):
                        ...
                        for r in rows:
                            if r:
                                texts.append(r.decode() if isinstance(r, (bytes,bytearray)) else r)
                        if not texts:
                            continue
                        if celery:
                            celery.send_task("persona.summarize_memory", args=[int(self.chat), texts, keys])
                    logger.info("_periodic_maintenance: scheduled summarize for %d old entries (t=%.3fs)",
