...
class RelevanceKnowledgeOwnerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_is_relevant_does_not_auto_accept_weak_tag_hits(self) -> None:
    ...
    async def test_is_relevant_accepts_strong_tag_hits_without_embedding_hits(self) -> None:
        async def _fake_find_tag_hits(*_args, **_kwargs):
    ...
    async def test_is_relevant_non_strict_keeps_legacy_keyword_auto_pass(self) -> None:
        async def _fake_find_tag_hits(*_args, **_kwargs):
            return [(0.10, "tag", "tag-hit")]
