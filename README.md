...
#### Ядро персоны (Persona Core)
- `app/emo_engine/registry.py` управляет lifecycle персон: получение/кэширование экземпляров, обновление cached-настроек и корректный shutdown.
- `app/emo_engine/persona/core.py` и `app/emo_engine/persona/brain.py` отвечают за поведение персоны, принятие решений и оркестрацию внутренних шагов ответа.
- `app/emo_engine/persona/memory.py` + `ltm.py` дают многослойную память (оперативная и долгосрочная), чтобы персона была консистентной между сообщениями.
- `app/emo_engine/persona/neurograph.py` и `states.py` формируют внутреннюю state-модель персоны и её динамику в диалоге.
- В API режиме профиль персоны из запроса нормализуется, хешируется в `persona_profile_id` и влияет на memory scope, чтобы изолировать контекст разных persona-конфигураций одного пользователя.
